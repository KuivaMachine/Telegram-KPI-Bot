...
import org.example.kpitelegrambot.data.entity.*;
...
public class StatisticService {
    ...
    public StatisticService(JdbcTemplate jdbcTemplate, LogService log) {
    ...
    private boolean makeSqlRequestByStatement(String sql) {
        try {
            ...
            log.error(String.format("НЕ УДАЛОСЬ ВЫПОЛНИТЬ ЗАПРОС %s - {%s}", sql, e.getMessage()));
            return false;
        }
    }
    /**
    ...
    public String getLastAddedPackerRecordToString() {
        ...
        try {
            ...
            log.info(String.format("СТАТИСТИКИ СБОРКИ НЕТ. ЗАПРОС %s ВЕРНУЛ ПУСТОЙ РЕЗУЛЬТАТ - %s", getLastStatRequest, e.getMessage()));
        }
        return null;
    }
    ...
    public String getLastAddedPrinterRecordToString(Employee currentEmployee) {
        ...
        try {
            ...
            log.info(String.format("СТАТИСТИКИ ПЕЧАТНИКА %s НЕТ. ЗАПРОС %s ВЕРНУЛ ПУСТОЙ РЕЗУЛЬТАТ - %s",currentEmployee.getFio(), getLastStatRequest, e.getMessage()));
        }
        return null;
    }
